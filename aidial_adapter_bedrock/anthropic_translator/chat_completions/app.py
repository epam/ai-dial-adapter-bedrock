...
async def _handle_messages(request: Request) -> Response:
    ...
    body: ChatCompletionRequest = to_chat_completions_request(req, deployment)
    ...
    if req.stream:
        ...
        return stream_response(translate_stream(events, deployment))

    response: ChatCompletion = await client.chat.completions.create(
        **data, stream=False, extra_headers=headers, extra_body=extra_body
    )
    return JSONResponse(
        content=from_chat_completions(response, deployment).model_dump(
            mode="json"
        )
    )
