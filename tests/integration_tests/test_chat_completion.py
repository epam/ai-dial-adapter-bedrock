...
from tests.utils import run_model, sanitize_test_name, user
...
def get_test_cases(
    ...
    ret.append(
        TestCase(
            ...
            streaming=streaming,
            messages=[user("")],
            test=Exception("Message content must not be empty"),
        )
    )
...
async def test_chat_completion_langchain(server, test: TestCase):
    model = create_model(BASE_URL, test.deployment.value, test.streaming)
