...
|AWS_SESSION_TAGS|NA|A JSON dictionary of the AWS STS session tags to pass when assuming a role, mapping the tag key to the field to take it from, for example `{"application":"Bedrock.modelId","project":"UserInfo.project"}`. Unset to disable.|
...
The variable is a JSON dictionary of `<tag key>: <field>` entries. The **key**
is the name the tag is passed to AWS under — pick whatever your IAM policies
expect. The **field** names where the value comes from:
...
Entries whose field is unknown (`Nope.project`, `Bedrock.region`, or an
...
a tag takes the `UserInfo.project` field and the user has a project, the session
is named `Project_<project>` instead — so add an entry for that field to
`AWS_SESSION_TAGS` to enable this. The tag key you choose doesn't matter here;
...
The `UserInfo.<path>` field takes the value at `<path>` from the JSON response
to the DIAL `GET /v1/user/info`
