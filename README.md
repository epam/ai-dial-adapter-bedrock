...
|AWS_SESSION_TAGS_ENABLED|false|Whether to pass AWS STS session tags when assuming a role.|
|AWS_SESSION_TAGS_USER_INFO_FIELDS|NA|Comma-separated list of paths into DIAL UserInfo to pass as session tags, for example `roles.0,project,userClaims.email`. Requires `DIAL_URL`. Unset to pass no UserInfo tags.|
...
Set `AWS_SESSION_TAGS_ENABLED=true` to pass AWS STS
[session tags](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_session-tags.html)
to the `AssumeRole` call made for a Bedrock client.
...
Tags come from independent sources, each contributing tag keys under its own
prefix, so that the sources can be told apart and their keys cannot collide.
The sources are configured separately, and a source that is unconfigured or
unavailable doesn't affect the tags contributed by the others.

|Source|Tag keys|Requires|
|---|---|---|
|Bedrock|`Bedrock.modelId`|Nothing beyond the feature being enabled|
|DIAL UserInfo|`UserInfo.<path>`|`DIAL_URL` and `AWS_SESSION_TAGS_USER_INFO_FIELDS`|
...
for details. Keys and values that exceed the length limits are truncated, and
tags beyond the 50-entry cap are dropped, both with a warning. Non-UserInfo
tags are never dropped by the cap.
...
`AWS_SESSION_TAGS_USER_INFO_FIELDS` is an allow-list of the fields to
take from the JSON response to the DIAL `GET /v1/user/info`
[request](https://dialx.ai/dial_api#operation/getUserInfo). The adapter resolves
the configured paths against that response and converts each resolved value to
a JSON string.

Leaving the variable unset passes no UserInfo tags and skips the user info
request entirely. The request also requires `DIAL_URL` to be set and the
incoming request to carry a DIAL API key; if either is missing, or if the
request fails, the UserInfo tags are skipped with a warning while the other
sources are still passed.
...
Tag keys are the configured paths prefixed with `UserInfo.`, e.g. the `roles.0`
path is passed as the `UserInfo.roles.0` tag. String values are used as-is. All
other values are JSON-serialized, e.g. numbers, booleans, `null`, objects and
arrays.

|Intent|`AWS_SESSION_TAGS_USER_INFO_FIELDS`|
|---|---|
|No UserInfo tags|Unset the variable|
|First role|`roles.0`|
|Project|`project`|
