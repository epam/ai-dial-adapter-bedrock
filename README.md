...
|CONVERSE_API_REQUEST_METADATA_FIELDS|NA|Comma-separated list of paths into DIAL User information to include in Converse API request metadata, for example `roles.0,project,userClaims.email`. Leave empty to disable. `*` wildcard is not supported.|
...
### Converse API Request Metadata

`CONVERSE_API_REQUEST_METADATA_FIELDS` configures optional
`requestMetadata` for Bedrock Converse and ConverseStream requests. The
variable is a comma-separated list of explicit paths into DIAL `UserInfo`.
Supported fields are:

|Field|Description|
|---|---|
|`roles`|User roles, addressable by list index, e.g. `roles.0`|
...
|Intent|`CONVERSE_API_REQUEST_METADATA_FIELDS`|
|---|---|
|Disabled|*(empty)*|
|First role|`roles.0`|
|Project|`project`|
