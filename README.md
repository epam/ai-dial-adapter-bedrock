...
|AWS_SESSION_TAGS|NA|Comma-separated list of AWS STS session tags to pass when assuming a role, each prefixed with the source to take it from, for example `Bedrock.modelId,UserInfo.roles.0,UserInfo.project`. Unset to disable.|
...
`AWS_SESSION_TAGS` lists the AWS STS
...
```ini
AWS_SESSION_TAGS=Bedrock.modelId,UserInfo.roles.0,UserInfo.project
```
...
Each entry is prefixed with the **source** to take the tag from, followed by
the field wanted from that source:
...
A `UserInfo.<path>` entry takes the field at `<path>` from the JSON response to
